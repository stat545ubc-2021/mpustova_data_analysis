...
ANSWER: **I would be most interested to do the flow_sample and vancouver_trees datasets. After using a glimpse() function to explore the datasets, I could see that both of these dataset contain data related to the environment - i.e. data related to the different species of trees and the data on stream/river flows**
...
**I would like to explore flow_sample dataset**
...
First, the dataset will be converted to a tibble:
...
As can be seen from the histogram, the most common values taken by variable "flow" are near-zero values.
...
Because of the distribution of the flow data, as well as the fact that data is split at two categorizes "maximum" and "minimum" by the categorical variable "extreme_type", the conclusion may be reached that the data only displays only the extreme flow events.


This assumption can be verified by creating a boxplot. The boxplot with a logarithmic scale is plotted to ensure a meaningful visual representation considering the considerable range of values of flow. 
...
The boxplot verified my assumption that the maximum and minimum categories correspond to maximum and minimum extreme flow events.


### Exploring Missing Values

First, the tibble should be created containing the number of missing values for each of the variables
...
Bar chart can be plotted to represent the missing_values per variable:
...
From this plot it can be derived that vew data is missing from columns "day", "flow", and "month".

Because of the large number of missing data in "sym" variable, it will be largely excluded from the analysis in the future.

### Exploring the relation between variables
I would like to explore the relation between month and extreme flow event. Particularly I would like to see during which months maximum flow usually occur, and during which months minimum flow occur.

#### Creating new tibble holding a subset of flow_sample data

Firsltm I would like **to create a tibble that will only hold the variables "flow", "month", and "extreme" and will not have any missing data** I will remove missing data after creating a subset to preserve as much data as possible. For example, if I removed the missing data before, I might have removed entire rows, where day or other variable would be missing. This would limit the analysis of the relation between month and flow.
...
Next, I would like to check whether the "month" column is categorical, and if not change its class so it is categorical
...
To explore how the extreme flow events are distributed across the months, I will create a bar chart.While it will not provide an insight on the magnitude of the individual flow events that have occurred each of the month, it provides an information on the number of extreme flow events that have occurred per month.
...
#### Exploring the characteristics extreme flow events observed in each of the months

It may also be valuable to look in more details at the values of flow by each of the month. Hence, as a next step of data analysis, I would like to present the relation between the month and the extreme flow in two graphs: One showing only maximum flow and another showing only minimum flow. This would allow to compare the magnitude of each category of the extreme flow events between the months. 

To achieve this, I will firstly create two more subsets: one for extreme_type "minimum" and one for extreme_type "maximum" **using the filter() function**
...
The following graph shows the extreme maximum flow events per months:
...
As can be seen from the graph, the majority and the highest magnitude of the maximum flow events occur in June. May and July have experience a smaller number of maximum flow events. Finally, August have only experienced one maximum flow event.

**Minimum flow:**
The following graph shows the extreme minimum flow events per months:
...
As can be seen from the graph, minimum flow events occurred in January, February, March, April, November and December. The lowest flow was observed in January, followed by February and March. 

##### Booxplot

Another way to visualize the values of flow observed in each of the months is by using boxplot.
...
As can be seen from the graph, the lowest mean flow was observed in February, while the lowest value even recorded occured in January.


Next, the same boxplot should be created for maximum flow:
```{r}
flow_max_boxplot <-ggplot(flow_subset_max, aes(month, flow))+ geom_boxplot(width = 0.2)+scale_y_log10()
print(flow_max_boxplot)
```

As can be seen from the graph, the highest mean of the maximum flow occurred in June. Moreover, June was the month when the highest flow was recorded,
