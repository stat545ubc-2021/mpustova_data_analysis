...
## Question 1: Have the maximum flow events increased in magnitude or frequency since 1909?

After further exploring the dataset, it becomes apparent that only one maximum and one minimum extreme weather event is recorded per each year:
```{r}
#Ordering the data by ascending year:
flow_sample_ordered <- flow_sample[order(flow_sample$year), ]
head(flow_sample_ordered)
```

The same can be (indirectly) demonstrated by comparing number of years and number of unique values in column year:

```{r}
dim(flow_sample)[1] #this shows the number of rows in the dataset
length(unique(flow_sample$year)) #shows the number of unique years in the dataset

```
As can be seen the number of rows is twice the number of years, hence confirming the assumption.

*Because of this, the following analysis will be focused on answering only the first part of the research question - i.e. Have the minimum flow events increase in magnitude* 


### Summarizing

To achieve this, I decided to do the following tasks for this research question: 
...
As have been outlined before, I will firstly create a new categorical variable "decade" which will hold information on which decade th data entries correspond to:
